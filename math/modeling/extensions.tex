...
$\mathrm{Draft} \mid \mathrm{Applied} \mid \mathrm{Reviewed} \mid \mathrm{Retired} \mid \mathrm{Abandoned}$.
The single forbidden transition is $\mathrm{Draft} \to \mathrm{Reviewed}$
without $\Witness$.
\end{theorem}
...
$\mathrm{Draft} \to \mathrm{Reviewed}$ without $\Witness$ closes a
...
$$\forall d.\ d.\mathtt{state} = \mathrm{Reviewed} \;\wedge\; d.\mathtt{witness} = \bot \;\Rightarrow\; S(d) = \mathrm{Fail}$$
...
A packet with $\Reg = \mathrm{Judgment}$ or $\Actor = \mathrm{Human}$
...
mode ($\mathrm{strict} / \mathrm{lenient} / \mathrm{off}$) is determined
...
\begin{itemize}
\item \mathrm{strict}: every commit of the packet has a valid GPG/SSH
      signature. Kernel rejects the packet without signature
      ($\mathrm{Fail}$).
\item \mathrm{lenient}: the \texttt{amend} commit has a signature;
      others need not. Kernel returns $\mathrm{Warn}$ if \texttt{amend}
      lacks signature.
\item \mathrm{off}: signatures not required; kernel does not check.
\end{itemize}
\end{theorem}