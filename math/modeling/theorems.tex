...
$$L(d, r) = \textsc{Drift} \iff d.\mathtt{witness} = \text{sha} \;\wedge\; \mathrm{prop}(r, \text{sha}, d.\mathtt{name}) \neq d.\mathtt{proposition}$$
...
the proposition is consistent with history and $L = \textsc{Applied}$
(possibly $\textsc{Stale}$ if files changed). If they differ, the
