---
name: math-coding
description: Record non-trivial design decisions as plain-text packets verified by a single OCaml binary (`mathc`). Use when proposing architecture, API shape, error strategy, or dependency choices. Skip for typos, renames, and one-line tweaks — for those a commit message is enough.
---
...
regenerated by `mathc render` and published at the GitHub Pages
site (`/skill.html`). Edit the source, not a snapshot.