#!/bin/sh
# tests/doc-version-consistency.sh — math-coding v0.993 version sync checker.
#
...
# v0.991/v0.993 mixed headers found in the v0.991 era.
...
EXPECTED="${EXPECTED_VERSION:-v0.993}"
...
docs/spec.md docs/when-not-to-use.md docs/migration-notes/v0.991-to-v0.993.md
theories/README.md
...
# Files that describe extensions and may carry their own version
# label (not the convention's). Listed here as ALLOWED versions
# for the file; any of these versions is acceptable.
ALLOWED_EXTRA_VERSIONS="
extensions/tdd.md:v0.992
extensions/obsidian.md:v0.992
extensions/ci/github-actions-tdd.yml:v0.992
core/spec/think-before-do.md:v0.992
core/spec/decision-modes.md:v0.992
core/agent/mathrc.sh:v0.992
core/README.md:v0.992
"
...
for f in $FILES; do
    ...
    if [ "$ver" != "$EXPECTED" ]; then
        # Allow extra versions for files in ALLOWED_EXTRA_VERSIONS.
        extra_ok=""
        for allowed in $ALLOWED_EXTRA_VERSIONS; do
            af="${allowed%%:*}"
            av="${allowed##*:}"
            if [ "$f" = "$af" ] && [ "$ver" = "$av" ]; then
                extra_ok=1
...
for f in $FILES; do
    ...
    ' "$abs")
    [ -z "$fm_ver" ] && continue
    # Allow short form (0.992) to match expected (v0.993).
    short_ver="v$fm_ver"
